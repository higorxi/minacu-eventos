...
export default function EventoDetalhesPage({ params }: { params: { id: string } }) {
  ...
  const evento = {
    id: params.id,
    nome: `Festival de Música de Minaçu ${params.id}`,
    data: "15/08/2024",
    horario: "18:00",
    local: "Parque Municipal de Minaçu",
    descricao: "O maior festival de música do interior de Goiás, reunindo artistas locais e nacionais em uma celebração da cultura musical brasileira.",
    imagens: [
    ...
    ingressos: [
      { tipo: "Pista", preco: "R$ 80,00", descricao: "Acesso à área geral do evento" },
      { tipo: "VIP", preco: "R$ 150,00", descricao: "Acesso à área VIP com vista privilegiada" },
      { tipo: "Camarote", preco: "R$ 250,00", descricao: "Acesso ao camarote com open bar" },
    ],
    regras: [
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div>
          <Tabs defaultValue="foto1" className="w-full">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="foto1">Foto 1</TabsTrigger>
              <TabsTrigger value="foto2">Foto 2</TabsTrigger>
              <TabsTrigger value="foto3">Foto 3</TabsTrigger>
            </TabsList>
            {evento.imagens.map((imagem, index) => (
              <TabsContent key={index} value={`foto${index + 1}`}>
                <img
                  src={imagem}
                  alt={`${evento.nome} - Foto ${index + 1}`}
          ...
          <div className="space-y-4 mb-6">
            ...
            <Dialog>
              ...
              <DialogContent>
                ...
                <div className="mt-4">
                  <p><strong>Capacidade:</strong> {evento.localInfo.capacidade}</p>
                  ...
                  <p><strong>Acessibilidade:</strong> {evento.localInfo.acessibilidade}</p>
                </div>
              </DialogContent>
            </Dialog>
          </div>
            ...
            <div className="space-y-4">
              {evento.ingressos.map((ingresso, index) => (
                <div key={index} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                  ...
                  <p className="text-lg font-bold text-primary mb-2">{ingresso.preco}</p>
                  <p className="text-gray-600 mb-4">{ingresso.descricao}</p>
                  <Button 
                    onClick={() => setSelectedTicket(ingresso.tipo)}
                    variant={selectedTicket === ingresso.tipo ? "default" : "outline"}
                    className="w-full"
                  >
                    {selectedTicket === ingresso.tipo ? "Selecionado" : "Selecionar"}
                  </Button>
                </div>
              ))}
            </div>
      ...
      <div className="mt-8 p-4 bg-yellow-100 rounded-lg">
        ...
        <p>Lembre-se de trazer um documento de identificação com foto e chegar com antecedência para evitar filas.</p>
      </div>
    </div>
  )
}
